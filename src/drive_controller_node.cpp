...
namespace bfr
{
      ...
      : Node("DriveController", options)
  {
    ...
    this->callbackHandle = this->add_on_set_parameters_callback(std::bind(
        &DriveControllerNode::parametersCallback, this, std::placeholders::_1));
  ...
  DriveControllerNode::parametersCallback(
    ...
    for (const auto &parameter : parameters)
      {
        ...
        if (parameter.get_name() == "manualControlAllowed" &&
          ...
          {
            rclcpp::SubscriptionOptions sub_options;
            sub_options.event_callbacks.liveliness_callback = std::bind(
                &DriveControllerNode::input_liveliness_changed, this, _1);
            sub_options.event_callbacks.deadline_callback = std::bind(
                &DriveControllerNode::input_deadline_changed, this, _1);
            ...
            if (this->manualControlAllowed == false)
              {
                ...
                this->cmdSubscription =
                    this->create_subscription<geometry_msgs::msg::Twist>(
                        "cmd_vel", 10,
                        std::bind(&DriveControllerNode::twist_callback, this,
                                  _1));
            ...
            } else
              {
                ...
                this->gamepadSubscription =
                    this->create_subscription<bfr_msgs::msg::Gamepad>(
                        "hal/inputs/gamepad", this->base_qos,
                        std::bind(&DriveControllerNode::gamepad_callback, this,
                                  _1),
                        sub_options);
              }
  ...
  void DriveControllerNode::input_liveliness_changed(
      rclcpp::QOSLivelinessChangedInfo &event)
  {
    std::cout << "Input liveliness changed! " << event.alive_count_change
              << std::endl;
  ...
  void DriveControllerNode::input_deadline_changed(
  ...
  void DriveControllerNode::gamepad_callback(
      const bfr_msgs::msg::Gamepad::SharedPtr msg)
  {
            ...
            if (msg->value > 10 || msg->value < -10)
              {
                outputVelocity = bfr_base::scale(
                    -100., 100., this->minVelocity.value,
                    this->maxVelocity.value, static_cast<double>(msg->value));

                outputRPH =
                    (outputVelocity * 100.) / this->wheelCircumference.value;
                outputRPM = outputRPH / 60.;
                outputTPS = outputRPM / 60.;

                output.data =
                    static_cast<float>(outputTPS / this->driveGearRatio);
  ...
  void DriveControllerNode::twist_callback(
  ...
  DriveControllerNode::tank_to_twist(double leftWheelVelocity,
                                     double rightWheelVelocity)
  {
    ...
    outputTwist.angular.z =
        (rightWheelVelocity - leftWheelVelocity) / this->wheelbase;

    return outputTwist;
  }

  std::vector<double>
  DriveControllerNode::twist_to_tank(geometry_msgs::msg::Twist input)
  {
  }
