...
namespace bfr
{
   ...
   */
  ArdakNode::ArdakNode(const rclcpp::NodeOptions &options)
      : Node("ardak", options)
  {
    ...
    this->runPublisher = this->create_publisher<std_msgs::msg::Bool>(
        "safety/run", this->base_qos);
    ...
    this->gpsSubscription =
        this->create_subscription<sensor_msgs::msg::NavSatFix>(
            "gps/data", 10, std::bind(&ArdakNode::gps_callback, this, _1));

    this->headingSubscription =
        this->create_subscription<sensor_msgs::msg::MagneticField>(
            "mag/data", 10, std::bind(&ArdakNode::heading_callback, this, _1));

    this->loopTimer =
        this->create_wall_timer(150ms, std::bind(&ArdakNode::loop, this));
  }
  ...
  void ArdakNode::heading_callback(
  ...
  {
    if (this->initialFix.status.status ==
